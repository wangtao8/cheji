...
Page({
   ...
   */
  data: {
    navbar: ['全部', '已下单', '已接车', '已完成', '已取消'],
    status: ['', 'CREATED', 'RECEPT', 'FINISH', 'CANCELED'],//navbar对应的状态值
  ...
  navbarTap: function (e) {//列表切换
    ...
    if (fenlei == 1) {
      ...
      _this.getOrderData2(0)
    }
  },
  getOrderData: function (e) { //客户查询接口
    var _this = this
    var userId = app.globalData.sessionId
    ...
    wx.request({
      url: api + 'api/v1/wx/bizMaintainPackageOrder/list',
      data: {
        'thirdSessionKey': userId,
        'limit': 4,
        'offset': e == 1 ? pageSizes : 0,
        'orderStatus': status[currentTab]
      },
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      },
  ...
  getOrderData2: function (e) { //修理工查询接口
    var _this = this
    var userId = app.globalData.sessionId
    var pageIndex = _this.data.pageIndex
    console.log('查询分类订单接口的当前页数：', pageIndex)
    ...
    var currentTab = this.data.currentTab
    wx.request({
      url: api + 'api/v1/wx/bizMaintainPackageOrder/push/list',
      data: {
        'thirdSessionKey': userId,
        'limit': 4,
        'offset': e == 1 ? pageSizes : 0,
  ...
  goInfo: function (e) {
    ...
    if (fenlei == 1) {
      ...
      wx.navigateTo({
        url: '../orderInfo/index?data=' + itemDataString
      })
    }
  ...
  onLoad: function (options) {
    ...
    console.log('页面的currentTab：', currentTab)
    this.setData({ currentTab: currentTab, fenlei: fenlei})
    this.initEleWidth()
  },
  ...
  onReachBottom: function () {
    ...
    if (pageSizes < pageAll) {
      ...
      var fenlei = this.data.fenlei
      if (fenlei == 1) {
        _this.getOrderData(1)
      } else {
        _this.getOrderData2(1)
      }
    } else {
      wx.showToast({
        title: '没有更多啦',
